...
function Homepage({ currentPage, setCurrentPage, UserName, setName }) {
  ...
  <div style={{
    ...
    <div className={`fade-text ${visible ? 'visible' : ''}`}>
      <p className={`verdana-xl padding-class-around`}>
        Welcome to Luna
      </p>
    </div>
    <div className={`fade-text2 ${visible ? 'visible' : ''}`}>
      <p className={`verdana-xs-italic padding-class-aroundish`}>
        A menstrual period predictor tool
      </p>
      <p className={`verdana-xs padding-class-aroundest`}>
      This tool will give you a well-informed prediction of the day your next period will start based on some information about you.
      </p>
      </div>
