...
# Ensure features is defined before the loop
features = np.array([[cycle_1, cycle_2, Age, bmi]])  # Re-define features if necessary

for i in range(n_iterations):
    # create a bootstrapped dataset
    indices = np.random.choice(len(features), len(features), replace=True)
    X_resampled, y_resampled = features[indices], np.array([prediction])[indices]
