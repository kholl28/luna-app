...
class BMICalculator(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        # BMI, Weight, and Height are at indices 6, 5, and 4 respectively      
        missing_bmi_mask = np.isnan(X[:, 6]) & ~np.isnan(X[:, 5]) & ~np.isnan(X[:, 4])
        X[missing_bmi_mask, 6] = X[missing_bmi_mask, 5] / ((X[missing_bmi_mask, 4] / 100) ** 2)  # Calculate BMI using NumPy array operations
