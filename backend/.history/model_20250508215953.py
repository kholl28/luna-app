...
X = df[['Cycle_1', 'Cycle_2', 'Age', 'Height', 'Weight', 'BMI']] # denote all predictos with uppercase X
# ensure that order of columns here matches order of columns in main project
y = df['Cycle_3'] # denote target (outcome variable) with lowercase Y
...
# Apply the BMICalculator function to X to remove missing values in BMI
bmi_calculator = BMICalculator()
X_array = bmi_calculator.fit_transform(X_array)

# scale X
scaler = StandardScaler()
X_array = scaler.fit_transform(X_array)

# Convert back to DataFrame with original column names and ensure numeric data types
X = pd.DataFrame(X_array, columns=X_original.columns)
