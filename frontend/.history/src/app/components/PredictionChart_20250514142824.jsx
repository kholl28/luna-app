...
const PredictionChart = ({ predictionDate, lowerBound, upperBound }) => {
  ...
  useEffect(() => {
    if (!predictionDate || !lowerBound || !upperBound) return;
    ...
    const predictedDateObjObj = parseISO(predictionDate);
    const lowerBoundObj = parseISO(lowerBound);
    const upperBoundObj = parseISO(upperBound);
    
    
    // Add extra days on both sides for visualization
    const startDate = addDays(lowerBoundObj, -1);
    const endDate = addDays(upperBoundObj, 1);
  ...
  const predictedIndex = findDateIndex(predictedDateObjObj);
