...
function Demos({ currentPage, setCurrentPage, UserName, setName, setAge, Age, Feet, setFeet, Inches, setInches, Weight, setWeight }) {
  ...
  const handleSubmit = (event) => {
    if (Age.length < 14 || Age.length > 55) {
      setDateError('Please enter a valid value for age.');
      return;
    } if (Feet.length < 4 || Feet.length > 7) {
      setDateError('Please enter a valid value for feet.');
      return;
    } if (Inches.length < 0 || Inches.length > 11) {
      setDateError('Please enter a valid value for inches.');
      return;
    } if (Weight.length < 60 || Weight.length > 400) {
      setDateError('Please enter a valid value for weight.');
      return;
    } else {
      setDateError('');
    }
    setCurrentPage("input");
    console.log("Submitted demographic valu")
  };
