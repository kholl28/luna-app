...
function Demos({ currentPage, setCurrentPage, UserName, setName, setAge, Age, Feet, setFeet, Inches, setInches, Weight, setWeight }) {
  ...
  const handleSubmit = (event) => {
    ...
    //   return;
    // } else if (Feet.length < 4 || Feet.length > 7) {
    //   setDemoError('Please enter a valid value for feet.');
    //   return;
    // } else if (Inches.length < 0 || Inches.length > 11) {
    //   setDemoError('Please enter a valid value for inches.');
    //   return;
    // } else if (Weight.length < 60 || Weight.length > 400) {
...
<div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', marginTop: '4rem' }}>
    <Button onClick={handleSubmit} className={`verdana-small ${Age.length > 0 && Feet.length >0? '' : 'hidden-text'}`}>
        Next
        </Button>
        </div>
