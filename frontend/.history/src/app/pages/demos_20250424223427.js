...
function Demos({ setCurrentPage, UserName, setAge, Age, Feet, setFeet, Inches, setInches, Weight, setWeight, setDemoError, demoError }) {
  useEffect(() => {
...
<div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', marginTop: '4rem' }}>
    ...
    <Button onClick={handleSubmit} className={`verdana-small ${Age && Feet && Inches && Weight ? '' : 'hidden-text'}`}>
</div>
  );
}
