...
function Input({ 
  ...
  const handleClick = async () => {
    ...
    // Calculate cycle lengths in days (not using formatDistance)
    ...
    try {
      const response = await fetch("http://127.0.0.1:5000/predict", {
        ...
        body: JSON.stringify({
          ...
          cycle_1: `${cycle1Days} days`,  // Send in the format the backend expects
          cycle_2: `${cycle2Days} days`,  // Send in the format the backend expects
          Age: Age,
          Feet: Feet,
          Inches: Inches,
